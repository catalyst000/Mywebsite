...
export default function Navbar() {
  ...
  return (
    <Box bg={isDark ? "gray.800" : "blue.600"} px={2} py={1.5} color="white" as="nav" h="60px" w="100%" position="fixed" top={0} left={0} zIndex={10} boxShadow="md">
      <Flex align="center"  justify="space-between" bg="transparent" h="100%">
        ...
        <HStack spacing={6} display={{ base: "none", md: "flex" }}>
          {links.map((link) => (
            <Link  fontFamily="'Playfair Display', serif" textDecoration="none" key={link.name} href={link.href}>
              {link.name}
            </Link>
          ))}
...
<Collapse in={isOpen} animateOpacity>
  ...
  >
    {links.map((link) => (
      <Link textDecoration="none" color="white" key={link.name} href={link.href} >
        {link.name}
      </Link>
    ))}
  </VStack>
